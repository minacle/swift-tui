...
struct ScrollIndicatorTests {
    ...
    func `dragging a thumb beyond the track clamps to the last offset`() {
        ...
        #expect(runtime.dispatch(PointerPress(button: .left, location: Point(column: 0, row: 0), phase: .down)) == .ignored)
        #expect(runtime.dispatch(PointerMotion(button: .left, location: Point(column: 19, row: 0), modifiers: [])) == .ignored)
        #expect(runtime.dispatch(PointerPress(button: .left, location: Point(column: 19, row: 0), phase: .up)) == .ignored)
    ...
    func `dragging from a mixed thumb cell preserves its heavy-half grab position`() {
        ...
        #expect(runtime.dispatch(PointerPress(button: .left, location: Point(column: 2, row: 0), phase: .down)) == .ignored)
        #expect(runtime.dispatch(PointerMotion(button: .left, location: Point(column: 3, row: 0), modifiers: [])) == .ignored)
        #expect(runtime.dispatch(PointerPress(button: .left, location: Point(column: 3, row: 0), phase: .up)) == .ignored)
