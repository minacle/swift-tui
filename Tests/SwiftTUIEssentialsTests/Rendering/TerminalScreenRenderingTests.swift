...
struct TerminalScreenRenderingTests {
    ...
    @Test
    func `TerminalScreenRenderer centers a content frame in the terminal viewport`() {
        let frame = TerminalScreenRenderer.frame(
            for: "Hello",
            in: TerminalViewportSize(columns: 400, rows: 240)
        )

        #expect(frame == RenderedTerminalFrame(text: "Hello", row: 120, column: 198))
    }
    ...
    func `a differential render writes only the changed character`() {
        let output = TerminalScreenRenderer.diff(
    ...
    func `redraw performs a full-screen render when the viewport changes`() {
        let output = TerminalScreenRenderer.redraw(
            from: RenderedBlock(lines: ["A"]),
